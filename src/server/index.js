...
app.get('/api/content/daily-ideas', async (req, res) => {
  try {
    ...
    const responseContent = await generateIdeas();
...
// Refresh Ideas endpoint
app.post('/api/content/refresh-ideas', async (req, res) => {
  try {
    ...
    console.error('Error generating fresh ideas:', error);
    res.status(500).json({
      success: false,
      error: {
        message: error.message,
